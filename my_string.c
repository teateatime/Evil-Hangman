...
int my_string_compare(MY_STRING hLeft_string, MY_STRING hRight_string){
    ...
    my_string* s1 = ((my_string*)hLeft_string); // error reading char strings
...
Status my_string_extraction(MY_STRING hMy_string, FILE* fp){
    ...
    // Do not make a new my string object
    // Never use fseek or ftell    
    ret = fscanf(fp, "%c", &ch);
    /* 
    if (ch == ' ' || ch == '\n') {
        ungetc(ch, fp);
    ...
    */
    while (ret != EOF && ch != ' ' && ch != '\n' && ch != '\t') {  // Outside Loop: clear leading whitespace 
	    ...
	    // free data first then point to new array which is called arr 
	    free(pMy_string->data);
            pMy_string->data = arr;
...
Status my_string_assignment(Item* pLeft, Item Right) {
    ...
    if (ppLeft == NULL) {
        //Create temp my_string handle
        //Create new string object
        //Create array of chars
        //set size and capacity of left to be right
        //Copy chars from right to left
        //store the new string object to left
        my_string* pTemp;
